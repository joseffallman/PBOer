    ...
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            switch (args.Length)
            {
                case 1:
                    folder = args[0];
                    new PBOSharpWrapper(new Configs.ConsolConfigs()).PackPBO(folder);
                    break;
                case 2:
                    folder = args[0];
                    exportTo = args[1];
                    new PBOSharpWrapper(new Configs.ConsolConfigs()).PackPBO(folder, exportTo);
                    break;
                case 3:
                    folder = args[0];
                    exportTo = args[1];
                    pboName = args[2];
                    new PBOSharpWrapper(new Configs.ConsolConfigs()).PackPBO(folder, exportTo, pboName);
                    break;
                default:
